...
async function ViewChatbots() {
  ...
  if (!userId) {
    console.error(
      "User ID is not available. Authentication might have failed."
    );
    return;
  }
  ...
  if (!response.data) {
    console.error("No data returned from the query", response.errors);
    return;
  }

  const { chatbotsByUser } = response.data;

  console.log("Chatbots by User Data:", chatbotsByUser);

  if (!chatbotsByUser) {
    console.error("chatbotsByUser is undefined");
    return;
  }
